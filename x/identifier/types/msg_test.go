...
func TestMsgUpdateIdentifier(t *testing.T) {
	tests := []struct {
		id          string
		controllers []string
		signer      string
	...
	for i, tc := range tests {
		...
		if tc.expectPass {
			require.Nil(t, msg.ValidateBasic(), "test: TestMsgUpdateIdentifier#%v", i)
		} else {
			require.NotNil(t, msg.ValidateBasic(), "test: TestMsgUpdateIdentifier#%v", i)
		}
	}
}
